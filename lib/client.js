window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var open = false;
    var reported = {};
    // iframe load watchdog state (see FRAME_LOAD_TIMEOUT_MS below).
    var frameLoaded = false;
    var frameRetried = false;
    var frameTimer = null;
    // The console's first paint can take several seconds (its /api/status fans out
    // to every account; cold call measured 7.4s). Allow a generous window before
    // we consider the load failed.
    var FRAME_LOAD_TIMEOUT_MS = 15000;
    // Consecutive failed probes required before the fallback is shown. Two strikes
    // (~16s apart) avoids flashing the error panel on a single transient blip.
    ...
    function setOpen(v) {
      ...
      if (open) {
        ...
        // Do NOT force a reload on every open: the console page + its /api/status
        // fan-out is slow (measured 7.4s cold) and re-issuing src made every open
        // look like "no response". Only load when the frame has no src yet; the
        // console refreshes its own data internally.
        loadFrame(false);
      } else {
    ...
    var CSS = [
      ...
      ".lad-fallback-btns{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;}",
      ...
      ".lad-again-alt{background:linear-gradient(180deg,#4a5568,#2d3748);}",
      // resize grip
    ...
    function buildUI() {
      ...
      // iframe "error" almost never fires in practice, so a plain load listener
      // left the fallback stuck on screen after a slow first paint. We instead
      // arm a watchdog on every src assignment: if the frame has not loaded
      // within FRAME_LOAD_TIMEOUT_MS we retry once, then fall back.
      ...
      // Panel recovery is automatic now (resident watchdog on the machine), so the
      // old "run node lobster-daddy.js panel yourself" instruction is gone — it
      // told users to do manual work that no longer applies.
      fallback.innerHTML = [
        "<div style='font-size:34px'>🦞</div>",
        "<b>控制台暂时没响应</b>",
        "<div>面板（<code>127.0.0.1:17819</code>）正在自动恢复，通常几秒内就好。<br>",
        "恢复守护每 60 秒巡检一次，会自动把它拉起来。</div>",
        "<div class='lad-fallback-btns'>",
        "<button class='lad-again' data-act='retry'>重新连接</button>",
        "<button class='lad-again lad-again-alt' data-act='external'>浏览器打开</button>",
        "</div>",
      ].join("");
    ...
    // Guarantees the drawer never stays stuck on the "没有响应" fallback when the
    // console is actually alive but simply slow: one silent retry, then fall back.
    ...
    function retryFrameOnce() {
      if (frameRetried) {
        // Second failure: the panel really is unreachable; show the fallback with
        // an accurate reason instead of leaving the user guessing.
        showFallback();
        return;
      }
      frameRetried = true;
      frameLoaded = false;
      try {
        if (frameEl) frameEl.setAttribute("src", PANEL_SRC + "?retry=" + Date.now());
      } catch (e) { /* ignore */ }
      armFrameWatchdog();
    }

    // Load the console through the proxy and start the watchdog.
    function loadFrame(force) {
      if (!frameEl) return;
      if (!force && frameLoaded) return;
      frameLoaded = false;
      try { frameEl.setAttribute("src", PANEL_SRC + (force ? "?t=" + Date.now() : "")); } catch (e) { /* ignore */ }
      armFrameWatchdog();
    }
    ...
    function bindDrawerEvents() {
      drawerEl.addEventListener("click", function (e) {
        ...
        if (act === "reload" || act === "retry") {
          hideFallback();
          frameRetried = false;      // an explicit user retry resets the budget
          frameLoaded = false;
          loadFrame(true);
          probe();
          return;
      ...
      try {
        fetch(STATUS_SRC, { cache: "no-store" })
          ...
          .then(function (s) {
            ...
            // Debounce: one slow/failed probe must not slap the fallback on screen.
            // The panel's cold /api/status took 7.4s in testing, and a single blip
            // used to raise the "没有响应" panel even though nothing was wrong.
            if (running) {
              probeFailStreak = 0;
              if (open) hideFallback();
            } else if (open) {
              probeFailStreak++;
              if (probeFailStreak >= 2) showFallback();
            }
          })
          .catch(function () {
            var dot = shadow.querySelector('[data-role="dot"]');
